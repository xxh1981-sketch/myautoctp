...
def _build_running_heartbeat_lines(
    ...
    *,
    ...
    if started is None:
        started = runtime.get('_process_started_at') or cfg.get('_process_started_at')
    if started and conn is not None:
        runtime.setdefault('_process_started_at', started)

    summary = build_runtime_health_summary(conn, cfg, ledger=ledger)
    lines = [
        f'timestamp={datetime.now().isoformat(timespec="seconds")} pid={os.getpid()}',
        'status=RUNNING',
    ]
    if summary.get('uptime_sec') is not None:
        lines.append(f'uptime_sec={int(summary["uptime_sec"])}')
    lines.extend(format_runtime_health_heartbeat_lines(summary))
    return lines
...
def _status_summary(conn, config: dict = None, ledger=None) -> str:
    ...
    summary = build_runtime_health_summary(conn, config or {}, ledger=ledger)
    open_unmatched = summary.get('open_unmatched')
    if open_unmatched is None:
        summary = dict(summary)
        summary['open_unmatched'] = _open_unmatched_count(ledger)
    return format_runtime_health_text(
        summary,
