...
**公开本仓时**：unit / lint 对 fork 可见；`pytest-full` 通过 secrets clone **私有** autotrade / autostraggle，外人 fork 无法复现 full job。详见 [PUBLIC_REPO.md](PUBLIC_REPO.md)。
...
| `pytest-unit` | push / PR | 无 autotrade | `python scripts/run_unit_tests.py`（38 个文件；`AUTOCTP_ALLOW_MISSING_DEPS=1` 注入 stub） |
...
# integration（需本地 autotrade / autostraggle，并设置 AUTOTRADE_ROOT）
